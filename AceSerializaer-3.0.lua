...
function Ace3test:TestAceSerializaer()
	self:TestBegin("AceSerializer")

	local inf = 1/0

	-- Usage: lua AceSerializer-3.0.lua [<burnin count, default 1>]
	local BURNIN = 1	-- roughly 1 sec execution time per loop, 10000 loops should be a good burn-in

	local AceSer = assert(LibStub("AceSerializer-3.0"))

	local function printf(fmt, ...)
		self:Printf(fmt, unpack(arg))
	end
	...
	-----------------------------------------------------------------------
	self:Print("> Test SerializeStringHelper")
	...
	-----------------------------------------------------------------------
	self:Print("> Test SerializeValue")
	...
	self:Print("> Test Strings")
	testsv("", "^S")
	testsv("hi", "^Shi")
	testsv("a\000b\032c", "^Sa~@b~`c")
	testsv("^S", "^S~\125S")

	self:Print("> Test other simply types")
	testsv(nil, "^Z")
	testsv(true, "^B")
	testsv(false, "^b")
	testsv(0, "^N0")
	...
	-----------------------------------------------------------------------
	self:Print("> Test Deserialize")

	self:Print("> Test Error testing")
	local ok, r1,r2,r3 = AceSer:Deserialize("errormoar")	-- plain error
	assert(not ok)
	assert(strfind(r1, "not AceSerializer data"))
	...
	assert(not ok)
	assert(strfind(r1, "misses AceSerializer terminator"), r1)

	self:Print("> Test empty data")
	local function x(ok,...)
		assert(ok)
	...
	self:Print("> Test simple datatypes")
	local ok, r1,r2,r3 = AceSer:Deserialize("^1^Sone^Stwo^^")	-- two strings -> ok
	assert(ok == true and r1=="one" and r2=="two" and r3==nil)

	local ok, r1,r2,r3 = AceSer:Deserialize("^1^B^b^^")	-- true, false -> ok
	assert(ok == true and r1==true and r2==false and r3==nil)

	local ok, r1,r2,r3 = AceSer:Deserialize("^1^Z^N5^^")	-- nil, 5 -> ok
	assert(ok == true and r1==nil and r2==5 and r3==nil)

	local ok, r1,r2,r3 = AceSer:Deserialize("^1^Nblurgh^^") -- invalid number -> error
	assert(not ok and strfind(r1,"Invalid serialized number"), r1)

	self:Print("> Test tables")

	local ok, r1,r2,r3 = AceSer:Deserialize("^1^T^t^^")	-- empty table
	assert(ok == true)
	assert(type(r1)=="table")
	assert(next(r1)==nil)
	assert(r2==nil)

	local ok, r1,r2,r3 = AceSer:Deserialize("^1^T^N1^Shi^t^^")	-- number = string
	assert(ok == true)
	assert(r1[1]=="hi")
	assert(r2==nil)

	local ok, r1,r2,r3 = AceSer:Deserialize("^1^T^T^Stheindex^Sisatable^t^T^Sthevalue^N2^t^t^Send^^")	-- table = table, with tacked on string to test iterator
	assert(ok == true)
	local k,v = next(r1)
	assert(type(k)=="table")
	assert(type(v)=="table")
	...
	assert(not ok and strfind(res, "misses AceSerializer terminator"))

	local ok,res = AceSer:Deserialize("^1^T^^")
	assert(not ok and strfind(res, "no table end marker"))
	...
	self:Print("> Test serializing and then de-serializing strings")
	...
	if BURNIN>1 then
		self:Print("String burn-in test:")
	end
	for b=1,BURNIN do
		for i=1,1e4 do
	...
	-----------------------------------------------------------------------
	self:Print("> Test wild combos, now as a table")
	...
	local ok,r = AceSer:Deserialize(ser)
	assert(ok == true)
	assert(r[1]=="firstval")
	assert(r[2]==1.23e-15)
	assert(r[3]==true)
	...
	self:Print("> Test floating-point accuracy")

	local function testone(v)
		local ser = AceSer:Serialize(v)
		local ok,deser = AceSer:Deserialize(ser)
		assert(ok and deser==v)
	end

	local function testone_tostr(v)
		local ser = AceSer:Serialize(v)
		local ok,deser = AceSer:Deserialize(ser)
		assert(ok and tostring(deser)==tostring(v))
	end

	local __myrand_n = 0
	local function myrand()
		__myrand_n = mod(__myrand_n + 1.23456789, 123)	-- this prng does not repeat for at least 10G iterations - tested up to 13.048G
	...
	if BURNIN>1 then
		self:Print("Floating point precision burn-in test:")
	end
	...
	local ok,res = AceSer:Deserialize(ser)
	assert(ok == true)
	local n1=0
